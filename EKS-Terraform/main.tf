...
resource "aws_subnet" "orghsk_subnet" {
  ...
  cidr_block              = cidrsubnet(aws_vpc.orghsk_vpc.cidr_block, 8, count.index)
...
resource "aws_route_table" "orghsk_rt" {
  ...
  route {
    ...
    gateway_id = aws_internet_gateway.orghsk_igw.id
  }
...
resource "aws_eks_cluster" "orghsk-eks" {
  name     = "orghsk-cluster"
  role_arn = aws_iam_role.orghsk_cluster_role.arn
  vpc_config {
...
resource "aws_eks_node_group" "orghsk-ng" {
  ...
  node_role_arn   = aws_iam_role.orghsk_node_group_role.arn
  subnet_ids      = aws_subnet.orghsk_subnet[*].id
